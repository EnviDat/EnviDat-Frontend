<template>
  <v-card id="MetadataFunding">
    ...
    <v-card-text
      ...
      ref="funding"
      :style="
        `overflow-x: hidden; scrollbar-color: ${scrollbarColorFront} ${scrollbarColorBack}`
      "
    ...
    >
      ...
      <v-row>
        ...
        >
          ...
          <v-row
            v-if="showFundingItem(item) && item.grant_number"
            no-gutters
            class="pt-2"
          >
    ...
    <v-card-text v-if="showPlaceholder && !funding" class="pa-4 pt-0">
      <div
        class="skeleton skeleton-size-normal skeleton-color-concrete skeleton-animation-shimmer"
      >
  ...
  computed: {
    ...
    emptyText() {
      return this.mixinMethods_getGenericProp(
        'emptyText',
        'No information about funding available for this dataset.',
      );
    },
    scrollbarColorFront() {
      return this.$vuetify
