  ...
  >
    <div id="credits">
      <a
        ...
        href="https://cesium.com/cesiumjs/"
        target="_blank"
        >CesiumJS</a
      >
      <a
        ...
        target="_blank"
        v-if="baseMapLayerName === 'satellite'"
        >Images &copy; Bing Maps</a
      >
      <a
        href="https://www.openstreetmap.org/"
        target="_blank"
        v-if="baseMapLayerName === 'streets'"
        >OpenStreetMap</a
      >
    </div>
  </div>
...
// import Viewer from 'cesium/Build/Cesium/Widgets/Viewer/Viewer';
...
export default {
  ...
  methods: {
    setupMap() {
      ...
      const cesiumWidgets = document.getElementsByClassName(
      ...
      this.viewer.scene.canvas.addEventListener(
        ...
        event => {
          ...
          if (selectedLocation) {
            const wgs = Ellipsoid.WGS84.cartesianToCartographic(
              selectedLocation,
            );
            // console.log(CesiumMath.toDegrees(wgs.latitude), CesiumMath.toDegrees(wgs.longitude));
            // console.log(viewer.scene);

            const posUL = viewer.camera.pickEllipsoid(
              new Cartesian2(0, 0),
              Ellipsoid.WGS84,
            );
            const posLR = viewer.camera.pickEllipsoid(
              new Cartesian2(viewer.canvas.width, viewer.canvas.height),
              Ellipsoid.WGS84,
            );
            const posLL = viewer.camera.pickEllipsoid(
              new Cartesian2(0, viewer.canvas.height),
              Ellipsoid.WGS84,
            );
            const posUR = viewer.camera.pickEllipsoid(
              new Cartesian2(viewer.canvas.width, 0),
              Ellipsoid.WGS84,
            );
      ...
      GeoJsonDataSource.load(geoJson).then(dataSource => {
        ...
        entities.forEach(entity => {
          ...
          if (isPoints) {
            ...
            // Set polygon style
            entity.polygon.material = new Color.fromCssColorString(
              this.color,
            ).withAlpha(0.5);
            entity.outline = true;
            // entity.outlineWidth = this.outlineWidth;
            entity.polygon.outlineColor = new Color.fromCssColorString(
              this.color,
            );
          }
    ...
    replaceLayer() {
      ...
      this.mapLayer = this.viewer.imageryLayers.addImageryProvider(
        cesiumLayer(this.wmsLayer),
      );
    },
    replaceBasemap() {
