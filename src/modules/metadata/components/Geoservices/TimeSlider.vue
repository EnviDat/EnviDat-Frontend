    ...
    <v-row align="center" style="height: 80%;" no-gutters>
      ...
      <v-col style="max-width: 50px;">
        <v-btn
          ...
          small
          @click="next()"
          :disabled="this.currentIndex >= this.nDataPoints - 1"
        >
...
export default {
  ...
  methods: {
    next() {
      if (this.currentIndex < this.nDataPoints - 1 && this.chart) {
        this.$emit(
          'select',
          this.chart.chart.dataProvider[this.currentIndex + 1].name,
        );
      }
    },
    previous() {
  ...
  computed: {
    currentIndex() {
      return this.chart
        ? this.chart.chart.dataProvider.findIndex(
            dataPoint => dataPoint.name === this.selected,
          )
        : null;
    },
    currentName() {
      return this.chart
        ? this.chart.chart.dataProvider[this.currentIndex].name
        : null;
    },
    nDataPoints() {
