<template>
  ...
  >
    <v-container fluid class="pa-4">
      ...
      <DataCreditLayout
        v-if="authorDetailsConfig.showDataCredits"
        ...
        <v-col class="pa-1" cols="6">
          <v-row no-gutters>
            ...
            >
              <a
                v-if="
                  (author.id.type && author.id.type === 'orcid') ||
                    isOrcId(formatIdentifier(author.id.identifier))
                "
                :href="
                  `https://orcid.org/${formatIdentifier(author.id.identifier)}`
                "
...
export default {
  ...
  computed: {
    ...
    authorIsDead() {
      return this.asciiDead && this.author.fullName
        ? this.author.fullName.match(this.asciiDead)
        : false;
    },
  },
  methods: {
