<template>
  <v-card ripple hover style="height: 100%; max-height: 375px !important;">
    <v-img
      background-color="primary"
      :height="
        flatLayout ? '65px' : $vuetify.breakpoint.smAndDown ? '100px' : '125px'
      "
    >
      <v-container class="fill-height py-0" style="position: absolute;">
        <v-row>
    ...
    <v-card-text primary-title>
      ...
      <v-row class="fill-height" align="end" no-gutters>
        <v-col v-for="n in 3" :key="n" class="shrink">
          <tag-chip-placeholder py-0 class="envidatChip" />
        </v-col>
      </v-row>
    </v-card-text>
  </v-card>
